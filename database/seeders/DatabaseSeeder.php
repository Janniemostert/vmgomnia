    ...
    public function run()
    {
        ...
        foreach ($chunks as $chunk) {
            ...
            if (empty($ids) || empty($baseUrl)) {
                \Log::error('VMG Cars Import - company_id or base_url is missing.');
                continue;
            }
            ...
            if ($response->successful()) {
                ...
                if (empty($results) || !is_array($results)) {
                    \Log::error('No car data found or invalid response format.');
                    continue;
                }
                ...
                foreach ($results as $result) {
                    // Data formatting remains the same as in the original command
                    ...
                    $formattedCooling = match($cooling) {
                        'A' => 'Air',
                        'W' => 'Water',
                        default => 'Not Specified'
                    };
                
                    $formattedTransmission = match($transmission) {
                        'A' => 'Automatic',
                        'M' => 'Manual',
                        default => 'Not Specified'
                    };
                
                    $formattedFuelType = match($fuelType) {
                        'P' => 'Petrol',
                        'D' => 'Diesel',
                        'E' => 'Electric',
                        default => 'Not Specified'
                ...
                \Log::info("Imported {$importCount} cars in this chunk.");
            } else {
                \Log::error('Failed to fetch data from VMG API: ' . $response->status());
            }
        }

        \Log::info("Total cars imported: {$totalImportCount}");
    }
}